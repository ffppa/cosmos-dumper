    ...
    async with CosmosClient(url, credential=key) as client:
        ...
        async def import_file(container_name, file_path):
            ...
            try:
                active_tasks = set()
                buffer = []
                buffer_size = 5000  # Aumentato il buffer per uno shuffle più efficace
                ...
                # Utilizziamo ijson come standard per caricare il file in streaming
                with open(file_path, "rb") as f:
                    logger.info(f"Processing {file_path} using ijson streaming parser...")
                    
                    # Tentiamo di capire se è un array JSON o JSONL (oggetti multipli)
                    # leggendo il primo carattere non-vuoto
                    first_char = ""
