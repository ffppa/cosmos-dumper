...
def export_container_worker(args_container_folder):
    args, container_properties, export_folder = args_container_folder
...
def export_cmd(args):
    asyncio.run(export_cmd_async(args))
...
def import_file_worker(args_container_file):
    args, container_name, file_path = args_container_file
...
async def import_cmd_async(args):
    ...
    if args.workers > 1 and len(files_to_import) > 1:
        with mp.Pool(args.workers) as pool:
            results = pool.map(import_file_worker, worker_inputs)
            total_ru = sum(results)
    else:
        for inp in worker_inputs:
...
def import_cmd(args):
    import_cmd_async_wrapper(args)


def import_cmd_async_wrapper(args):
    asyncio.run(import_cmd_async(args))
